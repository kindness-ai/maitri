if command -v limine &>/dev/null; then
  sudo tee /etc/mkinitcpio.conf.d/maitri_hooks.conf <<EOF >/dev/null
...
EOF
  ...
  sudo cp $MAITRI_PATH/default/limine/default.conf /etc/default/limine
  ...
  sudo cp $MAITRI_PATH/default/limine/limine.conf /boot/limine.conf

  maitri-pkg-add limine-snapper-sync limine-mkinitcpio-hook

  # Only snapshot root — /home is user data; rolling it back loses user work
  if ! sudo snapper list-configs 2>/dev/null | grep -q "root"; then
    sudo snapper -c root create-config /
  fi
  sudo cp $MAITRI_PATH/default/snapper/root /etc/snapper/configs/root

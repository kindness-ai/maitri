...
show_log_tail() {
  if [[ -f $MAITRI_INSTALL_LOG_FILE ]]; then
    local log_lines=$((TERM_HEIGHT - LOGO_HEIGHT - 35))
    local max_line_width=$((LOGO_WIDTH - 4))

    tail -n $log_lines "$MAITRI_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "maitri installation stopped!"
  show_log_tail

  gum style "This command halted with exit code $exit_code:"
  show_failed_script_or_command

  gum style "Get help: https://github.com/kindness-ai/maitri/issues"
  ...
  while true; do
    ...
    case "$choice" in
    "Retry installation")
      bash ~/.local/share/maitri/install.sh
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$MAITRI_INSTALL_LOG_FILE"
      else
        tail "$MAITRI_INSTALL_LOG_FILE"
      fi
      ;;
    "Exit" | "")
      exit 1

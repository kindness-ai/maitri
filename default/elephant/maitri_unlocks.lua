--
-- Dynamic maitri Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the maitri-shipped Plymouth via
-- maitri-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs maitri-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "maitriunlocks"
NamePretty = "maitri Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/maitri/themes"
  local maitri_path = os.getenv("MAITRI_PATH") or ""
  local default_themes_dir = maitri_path .. "/themes"
  local default_preview = maitri_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "maitri-launch-floating-terminal-with-presentation "
                .. shell_escape("maitri-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "maitri-launch-floating-terminal-with-presentation "
        .. shell_escape("maitri-plymouth-reset"),
    },
  }
